...
func (runner *SSHRunner) RunSession(commands ...string) error {
	var (
		target = fmt.Sprintf("%s@%s", runner.user, runner.ip)
		args   = append([]string{
			"-p", runner.sshPort,
			"-i", runner.pem,
			target},
...
func (runner *SSHRunner) CopyFile(file io.Reader, remotePath, permissions string) error {
	// Open and read file
